...
function AddCardForm({ onAdd }) {
  ...
  async function handleAdd() {
    ...
    try {
      ...
      onAdd(buildCardEntry(card, { quantity, condition, foil }));
...
function ImportModal({ onClose, onImportCards }) {
  ...
  function switchTab(t) {
    setTab(t); setRawText(''); setParsed(null);
  }
  ...
  async function handleImport() {
    ...
    for (const entry of parsed) {
      try {
        ...
        if (!card) {
          if (entry.setCode && entry.collNum) {
            card = await getCardBySetNumber(entry.setCode, entry.collNum);
          } else {
            card = await getCardByName(entry.name);
          }
          lookupCache.set(cacheKey, card);
          await delay(150); // respect Scryfall's ~10 req/sec guideline
        }
              ...
              {tab === 'text' && (
                <>
                  <p className="import-hint">
                    Supports Moxfield, MTG Arena, MTGO, Archidekt, and any tool using the standard format.
                  </p>
...
SB: 2 Tormod's Crypt`
              ...
              {tab === 'csv' && (
                <>
                  <p className="import-hint">
                    Upload a Moxfield CSV export. Required columns:{' '}
                    <code>Count, Name, Edition, Condition, Language, Foil</code>
                  </p>
                  ...
                  {rawText && (
                    <p className="import-file-loaded">
              ...
              {parsed !== null && (
                <div className="import-preview">
                  <p className="import-preview-header">
                    {parsed.length === 0
                      ? 'No cards recognized — check the format above.'
                      : `${parsed.length} entr${parsed.length === 1 ? 'y' : 'ies'} parsed`}
                  </p>
                  {parsed.length > 0 && (
                    <div className="import-preview-scroll">
                      <table className="import-preview-table">
                        <thead>
                          <tr><th>Qty</th><th>Name</th><th>Set / #</th><th>Foil</th><th>Cond.</th><th>SB?</th></tr>
                        </thead>
                        <tbody>
                          {parsed.slice(0, 25).map((e, i) => (
                            <tr key={i} className={e.sideboard ? 'import-row-sb' : ''}>
                              ...
                              <td>
                                {e.setCode
                                  ? `${e.setCode.toUpperCase()}${e.collNum ? ` #${e.collNum}` : ''}`
                                  : '—'}
                              </td>
                              <td>{e.foil ? '✨' : ''}</td>
                              <td>{e.condition}</td>
                          ...
                          {parsed.length > 25 && (
                            <tr>
                              <td colSpan={6} className="import-preview-more">
                                …and {parsed.length - 25} more entries
                              </td>
                            </tr>
                          )}
                        </tbody>
            ...
            <div className="modal-footer">
              ...
              {parsed === null && (
                <button className="btn-primary" onClick={handleParse} disabled={!rawText.trim()}>
                  Preview Import
                </button>
              )}
              ...
              {parsed !== null && parsed.length === 0 && (
                <button className="btn-secondary" onClick={() => setParsed(null)}>
                  Try again
                </button>
              )}
            </div>
        ...
        {step === 'importing' && (
          <div className="modal-body import-importing">
            ...
            <p className="import-rate-note">
              Rate-limited to ~7 cards/sec to respect Scryfall's API guidelines.
            </p>
          </div>
        )}
        ...
        {step === 'done' && (
          <div className="modal-body import-done">
            ...
            <p className="import-done-count">
              Added <strong>{addedCount}</strong> card{addedCount !== 1 ? 's' : ''} to your collection
            ...
            {failed.length > 0 && (
              <div className="import-failed">
                ...
                <ul className="import-failed-list">
                  {failed.map((name, i) => <li key={i}>{name}</li>)}
                </ul>
                <p className="import-failed-hint">
                  Check spelling or try without the set code. Cards with non-English names
                  or unusual printings may need to be added manually.
                </p>
              </div>
            )}
            <div className="modal-footer">
    ...
    <div className="page-wrap">
      ...
      {cards.length === 0 && (
        <div className="empty-state">
          <p>No cards yet — add a card above or import a list.</p>
        </div>
      )}
